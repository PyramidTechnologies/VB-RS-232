...
' Changed the following class statement from "Friend" to "Public" to resolve the problem being unable
' to access the designer

Public Class Apex232
    ...
    Dim Master(8) As Byte 'Outgoing data string
    Dim Slave(11) As Byte 'Incoming data string
    Public Toggle As Byte '0 or 1 alternating "Ack" value
    Dim Csum As Byte 'Checksum accumulator
    Public NRFlag As Object 'Error flag
    ...
    Private Sub Apex232_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
        Dim ct As Object
        ...
        'PortSelect = 1 'default com 1  Commented to use what is checked on default
    ...
    Private Sub cmdStartStop_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdStartStop.Click
        'Activated when the Start/Stop button pressed; toggles run/stop states
        If StartStop = 0 Then
            StartStop = 1 'toggle to "running" state
            cmdStartStop.Text = "STOP" 'stop/start button text
            cmdStartStop.BackColor = System.Drawing.ColorTranslator.FromOle(RGB(255, 0, 0))
        ...
        If StartStop = 1 Then
            ...
            cmdStartStop.Text = "START" 'stop/start button text
            cmdStartStop.BackColor = System.Drawing.ColorTranslator.FromOle(RGB(0, 255, 0))
...
ss_end:
    ...
    ' Event chkBill.CheckStateChanged may fire when form is initialized. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="88B12AE1-6DE0-48A0-86F1-60C0686C026A"'
    Private Sub chkBill_CheckStateChanged(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles chkBill.CheckStateChanged
        Dim Index As Short = chkBill.GetIndex(eventSender)
...
chkb_end:
    ...
    Private Sub SClear()
        ...
        For ctr = 0 To 5
            lbIN(ctr).Text = "" 'Clear output string displays
            lbOUT(ctr).Text = "" 'Clear input string display
        Next ctr
    End Sub
...
inbad:
    ...
    Private Sub Monitor()
        ...
        If Credit > 0 And Stacked = 1 Then
            ...
            If Delay > 4 Then
                txBillCount(Credit).Text = CStr(Val(Trim(txBillCount(Credit).Text)) + 1) 'update count
                Delay = 0
            End If
        End If
    ...
    End Sub
    Private Sub hsPoll_Scroll(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.ScrollEventArgs) Handles hsPoll.Scroll
        Select Case eventArgs.Type
            Case System.Windows.Forms.ScrollEventType.EndScroll
                hsPoll_Change(eventArgs.NewValue)
        End Select
    End Sub
